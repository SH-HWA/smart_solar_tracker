...
for _, row in df_raw.iterrows():
    ...
    # 2차 곡선(quadratic) 스플라인 보간 함수 생성 (자연스러운 외삽 연장 포함)
        ...
        augmented_data.append(
            {
                ...
                "시": hour,
                "분": minute,
                "방위각": round(float(az_val), 2),  # 소수점 둘째 자리 반올림 적용
            }
        )

# 증강 완료된 데이터프레임 생성 (3개년 총 약 16만 행으로 경량 최적화)
...
print(f"📁 생성된 파일: {model_filename}")
print(f"💡 메인 서버 폴더로 이 파일만 복사해서 predict()를 가동하세요!")
print(f"==================================================")
